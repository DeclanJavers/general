...
Two consequences worth naming. First, `forecasting-tools` keeps a narrower role than the v2 draft gave it: Metaculus tournament plumbing (if entering FutureEval) and the wrapper for non-Claude ensemble members — the agentic research loop itself comes from the Agent SDK. Second, **billing** (verified against support.claude.com, 2026-07): **personal Agent SDK automation is covered by a Claude Pro/Max plan** — leave `ANTHROPIC_API_KEY` unset and the SDK inherits your Claude Code login, drawing on the plan's shared usage limits, so harness runs compete with your interactive sessions for the 5-hour/weekly caps. Anthropic announced, then paused on 2026-06-15 ("for now, nothing has changed"), a move to a dedicated monthly Agent SDK credit (Pro $20 / Max 5x $100 / Max 20x $200, overflow at standard API rates) — if it unpauses, the bot's ~$1–1.50/question consumption fits comfortably inside a Max credit at target throughput. The docs' restriction ("no claude.ai login") applies to third-party products built on users' credentials, not to your own solo bot. Keep the `max_budget_usd`/`max_turns` guards regardless, and keep an API key as the escape hatch — it isolates harness runs from your interactive caps and is required if this ever becomes a multi-user product.
...
- **Cost envelope:** winners' operating point is ~20–30 LLM calls ≈ $1–1.50/question; a serious quarter costs low hundreds to ~$2k, not $100k (D.6). **Billing (v2.1, corrected):** personal SDK automation runs on your Claude plan — leave `ANTHROPIC_API_KEY` unset so the SDK inherits the Claude Code login; usage draws from the plan's shared caps (§1 map covers the paused credit-bucket change and the API-key fallback). Cap each member with `max_budget_usd`, log `total_cost_usd`, and treat ensemble size as the consumption knob either way.
...
- The runner is plain Python (asyncio); the SDK appears only inside `forecast.py`. Harness runs draw on the Claude plan's shared usage limits (or an API key, if configured for isolation — §1 map), so the runner enforces a per-run budget from `config.yaml` on top of the per-member `max_budget_usd`, halts loudly when it's hit, and schedules heavy runs away from interactive working hours to avoid competing for the shared caps.
